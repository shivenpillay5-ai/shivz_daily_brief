...
from daily_brief.tools.email import (
    EmailAttachment,
    InlineImage,
...
class EmailToolTests(unittest.TestCase):
    ...
    @patch("daily_brief.tools.email.smtplib.SMTP")
    def test_send_email_can_attach_documents(self, smtp_mock) -> None:
        smtp = smtp_mock.return_value.__enter__.return_value
        config = _email_config()
        ...
        send_email(
            ...
            attachments=[
                EmailAttachment(
                    ...
                    subtype="pdf",
                    filename="checkers-specials.pdf",
                )
            ],
        )
        ...
        self.assertEqual(pdf_parts[0].get_filename(), "checkers-specials.pdf")
        self.assertEqual(pdf_parts[0].get_payload(decode=True), b"%PDF-1.4")
